function S16HostRegistration({ data, setData, onNext, onBack }) {
  ...
  const validators = {
    1: () => {
      const e = {};
      if (!data.hostFullName?.trim())
        e.hostFullName = 'שם מלא הוא שדה חובה';
      if (!data.hostPhone?.trim() || data.hostPhone.replace(/\D/g,'').length < 9)
        e.hostPhone = 'הכנס מספר טלפון תקין';
      if (!data.hostEmail?.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data.hostEmail))
        e.hostEmail = 'הכנס כתובת אימייל תקינה';
      return e;
    },
    2: () => {
      const e = {};
        ...
        {Array.from({ length: TOTAL_STEPS }, (_, i) => i + 1).map((n) => (
          <div key={n} className="flex flex-col items-center gap-1.5 flex-1">
            <div className="flex items-center w-full">
              <div className={clsx(
                'flex-1 h-px transition-all duration-400',
            ...
            <span className={clsx(
              ...
              internalStep === n
                ? 'text-brand-600 font-semibold text-xs'
                : 'text-warm-400 text-xs'
...
function S2Explain({ onNext, onBack }) {
  ...
  if (internalStep === 1) return (
    <div className="screen-enter min-h-screen flex flex-col px-5 py-8 max-w-md mx-auto">
      ...
      <SectionTitle
        icon="🏡"
        title="הרשמת משפחה מארחת"
        sub="בואו נכיר אתכם לפני שנפגיש אתכם עם חיילים 🕯️"
      />
      <Card className="mb-5 bg-brand-50 border-brand-100 flex gap-3 items-start">
        <span className="text-2xl flex-shrink-0">💛</span>
        <p className="text-sm text-brand-800 leading-relaxed">
          תודה שבחרתם לפתוח את ביתכם לחיילים. מלאו את הפרטים ונחזור אליכם בהקדם!
        </p>
      </Card>
      <Input
        label="שם מלא"
        value={data.hostFullName||''}
        onChange={set('hostFullName')}
      ...
      <Input
        ...
        hint="נשתמש בו ליצירת קשר לפני שישי"
      />
      <Input
        label="כתובת אימייל"
        type="email"
  ...
  if (internalStep === 2) return (
    <div className="screen-enter min-h-screen flex flex-col px-5 py-8 max-w-md mx-auto pb-10">
      ...
      <SectionTitle
        icon="🍽️"
        title="סוג אירוח ומיקום"
        sub="ספרו לנו מה אתם מציעים ואיפה אתם נמצאים"
      />
      <RadioGroup
        label="סוג אירוח מועדף"
        value={data.hostingType||''}
        onChange={set('hostingType')}
        options={[
      ...
      />
      {errors.hostingType && (
        <p className="text-xs text-red-500 font-medium -mt-3 mb-4">{errors.hostingType}</p>
      )}
      <Input
        label="כתובת בית / עיר"
        value={data.hostCity||''}
        onChange={set('hostCity')}
        placeholder="לדוגמה: חיפה, רחוב הרצל 12..."
        error={errors.hostCity}
        hint="כתובת מדויקת מאפשרת התאמה מיטבית לחיילים מקרוב"
      />
      <MultiCheck
        label="שפות שמדברים בבית"
        options={LANG_OPTIONS}
        values={data.hostLanguages||[]}
        onChange={val => set('hostLanguages')(val)}
      />
      <div className="mt-auto pt-4">
        <Btn onClick={advance}>המשך ←</Btn>
      </div>
  ...
  if (internalStep === 3) return (
    <div className="screen-enter min-h-screen flex flex-col px-5 py-8 max-w-md mx-auto pb-10">
      ...
      <SectionTitle
        icon="🕯️"
        title="אורח החיים"
        sub="פרטים אלו עוזרים לנו להתאים חיילים עם אותן ציפיות"
      />
      <RadioGroup
        label="רמת שמירת שבת"
        value={data.shabbatObservance||''}
        onChange={set('shabbatObservance')}
        options={[
      ...
      />
      {errors.shabbatObservance && (
        <p className="text-xs text-red-500 font-medium -mt-3 mb-4">{errors.shabbatObservance}</p>
      )}
      <Card className="mb-5">
        ...
        <div className="space-y-3.5">
          <CheckRow checked={!!data.hostCanSleep} onChange={set('hostCanSleep')}>
            <span>
              ...
              <br/>
              <span className="text-warm-400">יכולים לספק מקום שינה לחייל/ת בשבת</span>
            </span>
          </CheckRow>
          <CheckRow checked={!!data.hostCanTransport} onChange={set('hostCanTransport')}>
            <span>
              ...
              <br/>
              <span className="text-warm-400">יכולים להסיע מ/לבסיס או לתחנה קרובה</span>
            </span>
          </CheckRow>
        </div>
      </Card>
  ...
  return (
    <div className="screen-enter min-h-screen flex flex-col px-5 py-8 max-w-md mx-auto pb-10">
      ...
      <SectionTitle
        icon="🏠"
        title="אופי הבית"
        sub="כמה פרטים אחרונים ואתם מוכנים!"
      />
      <div className="mb-6">
        ...
        {errors.hostCapacity && (
          <p className="mt-1.5 text-xs text-red-500 font-medium">{errors.hostCapacity}</p>
        )}
      </div>
      <div className="mb-2">
        <p className="text-sm font-semibold text-gray-700 mb-1.5">תגיות אופי הבית</p>
        <p className="text-xs text-warm-400 mb-3">
          בחרו תגיות שמתארות את הבית שלכם — עוזר לחיילים להרגיש בנוח
        </p>
        <MultiCheck
          options={VIBE_TAGS}
          values={data.hostVibeTags||[]}
          onChange={val => set('hostVibeTags')(val)}
        />
      </div>
      ...
      <Card className="mb-5 bg-warm-50 border-warm-200">
        ...
        <div className="space-y-2">
          {[
            ...
            {
              label: 'אירוח',
              val: data.hostingType === 'friday_dinner' ? 'ארוחת ליל שישי'
                 : data.hostingType === 'shabbat_lunch' ? 'ארוחת צהריים שבת'
                 : data.hostingType === 'delivery'      ? 'משלוח לבסיסים'
                 : null,
            },
            {
              label: 'שבת',
              val: data.shabbatObservance === 'observant'   ? 'שומרי שבת'
                 : data.shabbatObservance === 'traditional' ? 'מסורתי'
                 : data.shabbatObservance === 'secular'     ? 'חילוני'
                 : null,
            },
          ...
          {data.hostVibeTags?.length > 0 && (
            <div className="flex justify-between text-xs items-start gap-2">
              ...
              <span className="font-medium text-gray-800 text-left leading-relaxed">
                {data.hostVibeTags
                  .map(t => VIBE_TAGS.find(v => v.value === t)?.label || t)
                  .join(' · ')}
              </span>
            </div>
          )}
      ...
      <Btn onClick={advance} className="text-base py-4">
        השלם ורשום פרופיל 🏡
      </Btn>
    </div>
  );
